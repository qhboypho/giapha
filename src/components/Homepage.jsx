...
function IncenseSmokeCanvas({ className = "incense-smoke-canvas" }) {
  ...
  useEffect(() => {
    ...
    const spawnSmoke = (width, height, now) => {
      ...
      const spawnCount = Math.random() > 0.42 ? 2 : 1;
      ...
      for (let i = 0; i < spawnCount; i += 1) {
        particles.push({
          ...
          startSize: 3.4 + Math.random() * 2,
          endSize: 20 + Math.random() * 16,
          peakAlpha: 0.36 + Math.random() * 0.16
        });
      }
    };
    ...
    const draw = (time = 0) => {
      ...
      for (let wisp = 0; wisp < 5; wisp += 1) {
        ...
        const lift = height * (0.38 + wisp * 0.022);
        const sway = 5 + wisp * 1.6;
        ...
        ctx.save();
        ctx.globalAlpha = 0.14 + wisp * 0.013;
        ctx.filter = "blur(2.8px)";
        ctx.strokeStyle = "rgba(255, 255, 255, 0.72)";
        ctx.lineWidth = 1.45 + wisp * 0.14;
        ctx.lineCap = "round";
        ctx.beginPath();
